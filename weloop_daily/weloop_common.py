...
TIME_ZONE={-9:"America/Anchorage",-8:"America/Vancouver",-7:"America/Phoenix",-6:"America/Winnipeg",-5:"America/Detroit",
           -4:"America/Santiago",-3:"America/Montevideo",-2:"as",-1:"as",0:"Atlantic/Reykjavik",
           1:"Europe/Paris",2:"Europe/Sofia",3:"Europe/Moscow",4:"Asia/Tehran",5:"Asia/Karachi",
           6:"Asia/Dhaka",7:"Asia/Bangkok",8:"Asia/Shanghai",9:"Asia/Tokyo",10:"Australia/Brisbane",
           11:"Australia/Melbourne", 12:"Asia/Kamchatka",-11:"sd",-10:"Pacific/Honolulu"}
...
#时区设置
def get_Localtime_by_zone1( ptime,zonehour):
    l=re.split(':|-| ', ptime)
    t=""
    if len(l)==5:
...
def get_Localtime_by_zone(ptime="2017-01-05 13:03:00", zonehour=8):

    year, month, day, hour, menute ,second= re.split(':|-| ', ptime)
    t = datetime.datetime(int(year), int(month), int(day), int(hour), int(menute),int(second))
    dt = t.replace(tzinfo=pytz.timezone("utc"))
    time_zone = pytz.timezone(TIME_ZONE[zonehour])  # 构造时区
    local_dt = dt.astimezone(time_zone)  # 设置时区
