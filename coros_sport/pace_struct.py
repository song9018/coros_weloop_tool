...
class sport_struct(object):
    ...
    def RECORD_SPORT_TAG_IDLE(self, pstr):
        print("RECORD_SPORT_TAG_IDLE")

    @print_pace_info
    def RECORD_SPORT_TAG_MAGIC(self, pstr):
        print("RECORD_SPORT_TAG_MAGIC")

    @print_pace_info
    def gps_struct(self, pstr):
        bit_list = [4, 4]
        (self.__tag, self.__num) = app_bitmap_read_bit(pstr, bit_list)
        self.__size = int(len(pstr) / 2)
        getattr(self, GPS_STRUCT[self.__size])(pstr)

    @print_pace_info
    def peroid_struct(self, pstr):
        bit_list = [4, 4, 5, 3, 1, 7]
        (self.__tag, self.__num, self.__peroid_type, self.__value_reserve, self.__value_tag,
         self.value_valid_num) = app_bitmap_read_bit(pstr, bit_list)
        self._data = pstr[6:]
    ...
    def sportinfo_struct(self, pstr):
        bit_list = [4, 4, 4, 4]
        (self.__tag, self.__num, self.__sport_type, self.__sport_state) = app_bitmap_read_bit(pstr, bit_list)
        self._data = pstr[4:]
    ...
    def record_gps_head_t(self, pstr):
        ...
        pstr = pstr[2:]
        byte_list = [4, 4, 4, 1]
        (self.__utc, self.__lon, self.__lat, self.__interval) = app_bitmap_read_byte(pstr, byte_list)
    ...
    def record_gps_diff_t(self, pstr):
        bit_list = [4, 4, 1, 11, 1, 11]
        (self.__tag, self.__num, self._lonsign, self._lon, self.__latsign, self.__lat) = app_bitmap_read_bit(pstr,
                                                                                                             bit_list)
        if self._lonsign == 0:
    ...
    def peroid_time_t(self, pstr):
        byte_list = [4, 1]
        (self.__utc, self.__interval) = app_bitmap_read_byte(pstr, byte_list)
    ...
    def sport_start_info_t(self, pstr):
        byte_list = [4, 1, 1, 4,1]
        (self.__startutc, self.__metric_inch, self.__timezone, self.__lap_dis_set,self.__iron_group) = app_bitmap_read_byte(pstr,
                                                                                                          byte_list)

        if eval('0x' + pstr[10:12]) < 48:
            sport_struct.timezone = int((eval('0x' + pstr[10:12])) / 4)
        else:
            sport_struct.timezone = int((eval('0x' + pstr[10:12]) - 256) / 4)  # 反码加1(解决西时区问题)
        
        
        info_list = ["模式:%s,运动类型:%s ,运动开始时间:%s ,公英制:%s ,时区:%s ,单圈距离设置:%skm" % (iron_group[int(self.__iron_group)],SPORT_TYPE[self.__sport_type],
                                                                         self.get_time(self.__startutc),
                                                                         INCH[self.__metric_inch], self.__timezone,
                                                                         self.__lap_dis_set / 100000)]
    ...
    def sport_stop_info_t(self, pstr):
        byte_list = [4, 1]
        (self.__stoputc, self.__save_flag) = app_bitmap_read_byte(pstr, byte_list)
    ...
    def sport_pause_info_t(self, pstr):
        byte_list = [4, 10]
        (self.__pauseutc, self.__reverse) = app_bitmap_read_byte(pstr, byte_list)
    ...
    def sport_resume_info_t(self, pstr):
        byte_list = [4, 10]
        (self.__resumeutc, self.__reverse) = app_bitmap_read_byte(pstr, byte_list)

        info_list = ["运动类型:%s ,运动恢复时间:%s" % (SPORT_TYPE[self.__sport_type],
                                             self.get_time(self.__resumeutc))]
        self.sport_file.write(str(info_list) + "\n")

    @print_pace_info
    def lap_run_info_t(self, pstr):
        byte_list = [4, 1, 4, 4, 4, 4, 1, 1]
        (self.__current_utc, self.__lap_index, self.__lap_duration, self.__lap_distance, self.__lap_step,
         self.__lap_kcal, self.__lap_avg_cadence, self.__lap_avg_heartrate) = app_bitmap_read_byte(pstr, byte_list)

        info_list1 = ["第%s圈数时间:%s，单圈用时：%ss,单圈距离：%skm，单圈步数：%s，单圈卡路里：%skcal，单圈平均步频：%s，"
                      "单圈平均心率：%s" % (self.__lap_index, self.get_time(self.__current_utc), self.__lap_duration,
    ...
    def lap_bicycle_info_t(self, pstr):
        byte_list = [4, 1, 4, 4, 4, 1, 1, 2]
        (self.__current_utc, self.__lap_index, self.__lap_duration, self.__lap_distance, self.__lap_kcal,
         self.__lap_avg_cadence, self.__lap_avg_heartrate, self.__lap_avg_power) = app_bitmap_read_byte(pstr, byte_list)
    ...
    def lap_swim_info_t(self, pstr):
        byte_list = [4, 1, 4, 4, 2, 1, 2, 4, 1]
        (self.__current_utc, self.__lap_index, self.__lap_duration, self.__lap_distance, self.__lap_pace,
         self.__lap_swim_type, self.__lap_stroke, self.__lap_kcal, self.__lap_avg_heartrate) = app_bitmap_read_byte(
            pstr, byte_list)
    ...
    def sport_run_summary_info_t(self, pstr):
        byte_list = [4, 4, 1, 4, 1, 4, 1, 4, 2, 2, 2, 2, 1, 1, 2, 2]
        (self.__total_distance, self.__sport_duration, self.__total_lap_num, self.__total_kcal, self.__avg_heartrate,
         ...
         self.__avg_step_len
         , self.__max_cadence, self.__max_heartrate, self.__min_heartrate, self.__max_pace, self.__avg_pace) = \
            app_bitmap_read_byte(pstr, byte_list)
    ...
    def sport_swim_summary_info_t(self, pstr):
        byte_list = [4, 4, 1, 4, 1, 4, 4, 2, 2, 1, 1, 2, 2]
        (self.__total_distance, self.__sport_duration, self.__total_lap_num, self.__total_kcal, self.__avg_heartrate,
         self.__hrm_vo2max, self.__total_stroke, self.__max_pace, self.__avg_pace, self.__max_strk_rate_len,
         self.__avg_strk_rate_len, self.__max_swolf_len, self.__avg_swolf_len) = \
            app_bitmap_read_byte(pstr, byte_list)
    ...
    def sport_bicycle_summary_info_t(self, pstr):
        byte_list = [4, 4, 1, 4, 1, 4, 1, 2, 2, 2, 2, 1, 1, 2, 2]
        (self.__total_distance, self.__sport_duration, self.__total_lap_num, self.__total_kcal, self.__avg_heartrate,
